...
def menu_pick(event):
    ...
    if selection == 2:
        ...
        def save_changed_row():
            id = int(id_entry.get())
            changed_df = change_value(load_csv_to_df("database_example"),item_id=id,column_name="type",new_value=type_entry.get())
            ...
            form_table(load_csv_to_df("database_example"))
        def index_search():
            """
            ...
            SEARCH_BTN = Button(sec_window, text="search", background="gray", foreground="black",command = lambda : show_data(id_search.get()))
            SEARCH_BTN.grid(row=2, column=1, sticky="nesw", padx=5, pady=5)
            id_search.grid(row=1, column=1, sticky="nesw", padx=5, pady=5)

        # BUTTONS
        CONFIRM_BTN = Button(main_window, text="save", background="gray", foreground="black",command = lambda : save_changed_row())
        CONFIRM_BTN.grid(row=4, column=6, sticky="nwe", padx=5, pady=5)
        #
        id_entry = Entry(main_window, text="id", background="gray", justify=CENTER)
        type_entry = Entry(main_window, text="type", background="gray", justify=CENTER)
        name_entry = Entry(main_window, text="name", background="gray", justify=CENTER)
